...
logger_name = env.get('BOT_NAME', default='sistema-monitoramentos-diario-agravos')
fastapi_logger.name = logger_name

desc = '<a href="https://simda.sms.fortaleza.ce.gov.br/simda/notificacao-negativa/tabela-semana" target="_blank">FONTE</a>'

tags_metadata = [
    {
        'name': 'sistema-monitoramentos-diario-agravos',
        'description': 'Consulta informações do sistema de monitoramento diário de agravos'
    }
]
...
#---------------------------- Application -------------------------------
app = FastAPI(
    title='REST API - Consulta informações do sistema de monitoramento diário de agravos', 
    description=desc, 
    debug=False, 
    openapi_tags=tags_metadata,
    openapi_url="/api/sistema-monitoramentos-diario-agravos/consulta/docs/openapi.json",
    docs_url='/api/sistema-monitoramentos-diario-agravos/consulta/docs'
)
...
#---------------------------- Endpoints -------------------------------
@app.get(
    path="/api/sistema-monitoramentos-diario-agravos/consulta", 
    tags=['sistema-monitoramentos-diario-agravos'],
    responses=responses)
async def get_consulta(ano: str = ano_requerido):    
    return await consulta.fetch(ano)
